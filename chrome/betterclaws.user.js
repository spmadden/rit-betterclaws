// ==UserScript==
// @name                Better Claws
// @namespace	        http://seanmadden.net/projects/betterclaws
// @description	        I'm sick of claws - so I made it better
// @include		https://claws.rit.edu/admintools/*
// ==/UserScript==

/**
 * http://www.openjs.com/scripts/events/keyboard_shortcuts/
 * Version : 2.01.B
 * By Binny V A
 * License : BSD
 */
shortcut = {
	'all_shortcuts':{},//All the shortcuts are stored in this array
	'add': function(shortcut_combination,callback,opt) {
		//Provide a set of default options
		var default_options = {
			'type':'keydown',
			'propagate':false,
			'disable_in_input':false,
			'target':document,
			'keycode':false
		}
		if(!opt) opt = default_options;
		else {
			for(var dfo in default_options) {
				if(typeof opt[dfo] == 'undefined') opt[dfo] = default_options[dfo];
			}
		}

		var ele = opt.target;
		if(typeof opt.target == 'string') ele = document.getElementById(opt.target);
		var ths = this;
		shortcut_combination = shortcut_combination.toLowerCase();

		//The function to be called at keypress
		var func = function(e) {
			console.log("DERP");
			e = e || window.event;
			
			if(opt['disable_in_input']) { //Don't enable shortcut keys in Input, Textarea fields
				var element;
				if(e.target) element=e.target;
				else if(e.srcElement) element=e.srcElement;
				if(element.nodeType==3) element=element.parentNode;

				if(element.tagName == 'INPUT' || element.tagName == 'TEXTAREA') return;
			}
	
			//Find Which key is pressed
			if (e.keyCode) code = e.keyCode;
			else if (e.which) code = e.which;
			var character = String.fromCharCode(code).toLowerCase();
			
			if(code == 188) character=","; //If the user presses , when the type is onkeydown
			if(code == 190) character="."; //If the user presses , when the type is onkeydown
			if(code == 191) character="/";

			var keys = shortcut_combination.split("+");
			console.log(keys);
			//Key Pressed - counts the number of valid keypresses - if it is same as the number of keys, the shortcut function is invoked
			var kp = 0;
			
			//Work around for stupid Shift key bug created by using lowercase - as a result the shift+num combination was broken
			var shift_nums = {
				"`":"~",
				"1":"!",
				"2":"@",
				"3":"#",
				"4":"$",
				"5":"%",
				"6":"^",
				"7":"&",
				"8":"*",
				"9":"(",
				"0":")",
				"-":"_",
				"=":"+",
				";":":",
				"'":"\"",
				",":"<",
				".":">",
				"/":"?",
				"\\":"|"
			}
			//Special Keys - and their codes
			var special_keys = {
				'esc':27,
				'escape':27,
				'tab':9,
				'space':32,
				'return':13,
				'enter':13,
				'backspace':8,
	
				'scrolllock':145,
				'scroll_lock':145,
				'scroll':145,
				'capslock':20,
				'caps_lock':20,
				'caps':20,
				'numlock':144,
				'num_lock':144,
				'num':144,
				
				'pause':19,
				'break':19,
				
				'insert':45,
				'home':36,
				'delete':46,
				'end':35,
				
				'pageup':33,
				'page_up':33,
				'pu':33,
	
				'pagedown':34,
				'page_down':34,
				'pd':34,
	
				'left':37,
				'up':38,
				'right':39,
				'down':40,
	
				'f1':112,
				'f2':113,
				'f3':114,
				'f4':115,
				'f5':116,
				'f6':117,
				'f7':118,
				'f8':119,
				'f9':120,
				'f10':121,
				'f11':122,
				'f12':123
			}
	
			var modifiers = { 
				shift: { wanted:false, pressed:false},
				ctrl : { wanted:false, pressed:false},
				alt  : { wanted:false, pressed:false},
				meta : { wanted:false, pressed:false}	//Meta is Mac specific
			};
                        
			if(e.ctrlKey)	modifiers.ctrl.pressed = true;
			if(e.shiftKey)	modifiers.shift.pressed = true;
			if(e.altKey)	modifiers.alt.pressed = true;
			if(e.metaKey)   modifiers.meta.pressed = true;
                        
			for(var i=0; k=keys[i],i<keys.length; i++) {
				//Modifiers
				if(k == 'ctrl' || k == 'control') {
					kp++;
					modifiers.ctrl.wanted = true;

				} else if(k == 'shift') {
					kp++;
					modifiers.shift.wanted = true;

				} else if(k == 'alt') {
					kp++;
					modifiers.alt.wanted = true;
				} else if(k == 'meta') {
					kp++;
					modifiers.meta.wanted = true;
				} else if(k.length > 1) { //If it is a special key
					if(special_keys[k] == code) kp++;
					
				} else if(opt['keycode']) {
					if(opt['keycode'] == code) kp++;

				} else { //The special keys did not match
					if(character == k) kp++;
					else {
						if(shift_nums[character] && e.shiftKey) { //Stupid Shift key bug created by using lowercase
							character = shift_nums[character]; 
							if(character == k) kp++;
						}
					}
				}
			}
			console.log(modifiers);
			if(kp == keys.length && 
						modifiers.ctrl.pressed == modifiers.ctrl.wanted &&
						modifiers.shift.pressed == modifiers.shift.wanted &&
						modifiers.alt.pressed == modifiers.alt.wanted &&
						modifiers.meta.pressed == modifiers.meta.wanted) {
				callback(e);
	
				if(!opt['propagate']) { //Stop the event
					//e.cancelBubble is supported by IE - this will kill the bubbling process.
					e.cancelBubble = true;
					e.returnValue = false;
	
					//e.stopPropagation works in Firefox.
					if (e.stopPropagation) {
						e.stopPropagation();
						e.preventDefault();
					}
					return false;
				}
			}
		}
		this.all_shortcuts[shortcut_combination] = {
			'callback':func, 
			'target':ele, 
			'event': opt['type']
		};
		//Attach the function with the event
		if(ele.addEventListener) ele.addEventListener(opt['type'], func, false);
		else if(ele.attachEvent) ele.attachEvent('on'+opt['type'], func);
		else ele['on'+opt['type']] = func;
	},

	//Remove the shortcut - just specify the shortcut and I will remove the binding
	'remove':function(shortcut_combination) {
		shortcut_combination = shortcut_combination.toLowerCase();
		var binding = this.all_shortcuts[shortcut_combination];
		delete(this.all_shortcuts[shortcut_combination])
		if(!binding) return;
		var type = binding['event'];
		var ele = binding['target'];
		var callback = binding['callback'];

		if(ele.detachEvent) ele.detachEvent('on'+type, callback);
		else if(ele.removeEventListener) ele.removeEventListener(type, callback, false);
		else ele['on'+type] = false;
	}
}

var dialogBg;
var container;
var content;
window.addEventListener('load', function(){
	dialogBg = document.createElement("div");
	console.log("debug", dialogBg);
	if(dialogBg instanceof HTMLDivElement){
		dialogBg.appendChild(document.createTextNode("DERP DERP"));
		dialogBg.style.position='absolute';
		dialogBg.style.top = 0;
		dialogBg.style.left=0;
		dialogBg.style.width="100%";
		dialogBg.style.height="100%";
		dialogBg.style.backgroundColor = "#000000";
		dialogBg.style.zIndex = 10000;
		dialogBg.style.opacity = .8;
		dialogBg.style.display="none";
		dialogBg.innerHTML='<div id="dialogWrapper" style="background:#FFF;padding:10px;position:relative;top:30%;left:30%;height:40%;width:40%;opacity:1"><div id="dialogContent"></div><div id="loading" style="display:none"><img src="data:image/gif;base64," /></div><div id="longtime" style="display:none">Taking a long time?  Yeah.  It\'s goddamn claws.  Deal.</div></div>';
	}
	container = document.getElementById("container");
	container.parentNode.insertBefore(dialogBg, container);

	content = document.getElementById("dialogContent");
}, false);



var lastTimeOut = 0;
var lastKey = '';
var loading = function(e){
	document.getElementById("hostForm").removeEventListener('submit', loading, true);
	setTimeout(function(){
		document.getElementById("longtime").style.display="block";
	}, 10000);
	setTimeout(function(){
		document.getElementById("hostForm").submit();
	}, 200);
	document.getElementById("loading").style.display="block";
	return true;
}
var host = function(){
	content.innerHTML = 'Search by Hostname: <form method="GET" action="/admintools/computers/computerSearch.php" id="hostForm"><input id="DDNSHostname" name="DDNSHostname" type="text"/><input type="hidden" name="ACTION" value="SEARCH" /><input type="submit" value="SEARCH" /></form>';

}
var mac = function(){
	content.innerHTML = 'Search by Mac Address: <form method="GET" action="/admintools/computers/computerSearch.php" id="hostForm"><input id="DDNSHostname" name="MAC" type="text"/><input type="hidden" name="ACTION" value="SEARCH" /><input type="submit" value="SEARCH" /></form>';

}

var dhcp = function(){
	content.innerHTML = 'Search DHCP Log for: <form method="GET" action="/admintools/computers/logSearch.php" id="hostForm"><input id="DDNSHostname" name="STRING" type="text"/><input type="hidden" name="file" value="dhcpd.log" /><input type="hidden" name="ACTION" value="SEARCH" /><input type="submit" value="SEARCH" /></form>';

}

var account = function(){
	content.innerHTML = 'Search for DCE: <form method="GET" action="/admintools/users/useredit.php" id="hostForm"><input id="DDNSHostname" name="USERNAME" type="text"/><input type="hidden" name="ACTION" value="GETUSER" /><input type="submit" value="SEARCH" /></form>';
}

var subnets = function(){
	content.innerHTML = 'Search for subnet: <form method="GET" action="/admintools/computers/subnetManage.php" id="hostForm"><input id="DDNSHostname" name="ID" type="text"/><input type="hidden" name="ACTION" value="GET" /><input type="submit" value="SEARCH" /></form>';
}

var help = function(e){
	if(dialogBg.style.display=="block"){
		dialogBg.style.display="none";
		return;
	}
	dialogBg.style.top = window.pageYOffset + "px";
	dialogBg.style.display="block";
	content.innerHTML = '<p>Welcome to Better Claws!  I got sick and tired of the back-asswards claws interface so I made it ajaxy and shiny!</p><p>s+h : search computer by hostname<br />s+d : search latest DHCP logs<br />s+n : search subnets<br />s+a : search accounts (usernames)<br />s+m : search computers by mac address</p>';
}

var setup = function(fn){
	console.log("user typed " +lastKey);
	if(lastKey == 's'){
		dialogBg.style.top = window.pageYOffset + "px";
		dialogBg.style.display="block";
		fn();
		document.getElementById("hostForm").addEventListener("submit", loading, true);
		document.getElementById("DDNSHostname").focus();
	}
}

var search = function(){
	console.log("called search");
	clearTimeout(lastTimeOut);
	lastTimeOut = setTimeout(function(){
		console.log("resetting timeout");
		lastKey = '';
	}, 3000);
	lastKey = 's';
}
var clear = function(){
	dialogBg.style.display="none";
}
shortcut.add("s", search, {'disable_in_input':true});
shortcut.add("h",function(e){setup(host)}, {'disable_in_input':true});
shortcut.add('m',function(e){setup(mac)}, {'disable_in_input':true});
shortcut.add('d',function(e){setup(dhcp)},{'disable_in_input':true});
shortcut.add('a',function(e){setup(account)},{'disable_in_input':true});
shortcut.add('n',function(e){setup(subnets)},{'disable_in_input':true});
shortcut.add(".",help,{'disable_in_input':true});
shortcut.add("Escape", clear);

