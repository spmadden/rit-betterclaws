// ==UserScript==
// @name                Better Claws
// @namespace	        http://seanmadden.net/projects/betterclaws
// @description	        I'm sick of claws - so I made it better
...
dialogBg.appendChild(document.createTextNode("DERP DERP"));
...
var loading = function(e){
	document.getElementById("hostForm").removeEventListener('submit', loading, true);
	setTimeout(function(){
		document.getElementById("longtime").style.display="block";
	}, 10000);
	setTimeout(function(){
...
var host = function(){
	content.innerHTML = 'Search by Hostname: <form method="GET" action="/admintools/computers/computerSearch.php" id="hostForm"><input id="DDNSHostname" name="DDNSHostname" type="text"/><input type="hidden" name="ACTION" value="SEARCH" /><input type="submit" value="SEARCH" /></form>';

}
var mac = function(){
	content.innerHTML = 'Search by Mac Address: <form method="GET" action="/admintools/computers/computerSearch.php" id="hostForm"><input id="DDNSHostname" name="MAC" type="text"/><input type="hidden" name="ACTION" value="SEARCH" /><input type="submit" value="SEARCH" /></form>';

}

var dhcp = function(){
	content.innerHTML = 'Search DHCP Log for: <form method="GET" action="/admintools/computers/logSearch.php" id="hostForm"><input id="DDNSHostname" name="STRING" type="text"/><input type="hidden" name="file" value="dhcpd.log" /><input type="hidden" name="ACTION" value="SEARCH" /><input type="submit" value="SEARCH" /></form>';

}

var account = function(){
	content.innerHTML = 'Search for DCE: <form method="GET" action="/admintools/users/useredit.php" id="hostForm"><input id="DDNSHostname" name="USERNAME" type="text"/><input type="hidden" name="ACTION" value="GETUSER" /><input type="submit" value="SEARCH" /></form>';
}

var subnets = function(){
	content.innerHTML = 'Search for subnet: <form method="GET" action="/admintools/computers/subnetManage.php" id="hostForm"><input id="DDNSHostname" name="ID" type="text"/><input type="hidden" name="ACTION" value="GET" /><input type="submit" value="SEARCH" /></form>';
}

var name = function(){
	content.innerHTML = 'Search for People with names containing: 	<form method="GET" action="/admintools/users/usersearch.php" id="hostForm">	<input id="DDNSHostname" name="ldap_TmFtZQ==" type="text"/>	<input type="hidden" name="ldap_people" value="all" />	<input type="hidden" name="ldap_op_TmFtZQ==" value="Contains"/>	<input type="hidden" name="ldap_search_type" value="multi" />	<input type="submit" value="SEARCH" />	</form>';
}

var help = function(){
	if(dialogBg.style.display=="block"){
		dialogBg.style.display="none";
		return;
	}
	dialogBg.style.top = window.pageYOffset + "px";
	dialogBg.style.display="block";
	content.innerHTML = '<p>Welcome to Better Claws!  I got sick and tired of the back-asswards claws interface so I made it ajaxy and shiny!</p><p>s+h : search computer by hostname<br />s+d : search latest DHCP logs<br />s+n : search subnets<br />s+a : search accounts (usernames)<br />s+m : search computers by mac address<br />s+i : search by identity (users\' name contains)</p>';
}
...
var setup = function(fn){
	if(lastKey == 's'){
		...
		dialogBg.style.display="block";
		fn();
		document.getElementById("hostForm").addEventListener("submit", loading, true);
		document.getElementById("DDNSHostname").focus();
	}
}
...
shortcut.add("s", search, {'disable_in_input':true});
shortcut.add("h",function(){setup(host)}, {'disable_in_input':true});
shortcut.add('m',function(){setup(mac)}, {'disable_in_input':true});
shortcut.add('d',function(){setup(dhcp)},{'disable_in_input':true});
shortcut.add('a',function(){setup(account)},{'disable_in_input':true});
shortcut.add('i',function(){setup(name)},{'disable_in_input':true});
shortcut.add('n',function(){setup(subnets)},{'disable_in_input':true});
shortcut.add(".",help,{'disable_in_input':true});
shortcut.add("Escape", clear);
